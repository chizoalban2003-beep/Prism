...
class MatchReportTask(TaskExecutor):
    ...
    def _build_prompt(self, ctx: ExecutionContext, summary_only: bool = False) -> str:
        ...
        sections = "sections: ## Performance, ## Tactics, ## Improvements" if not summary_only else "3-paragraph summary"
        return (
...
class SocialMediaTask(TaskExecutor):
    ...
    def safe(self, ctx: ExecutionContext) -> ExecutionOutcome:
        ...
        template = {
            ...
            "instagram": "Another day, another grind 💪 [highlight]. Feeling [tone] after this [session_type]. #[sport]",
            "linkedin":  "Proud to share another milestone in my [sport] journey. [highlight]. Grateful for the process.",
...
<p>Recovery score: <span class="badge {'green' if metrics.get('recovery_score', 0.7) > 0.6 else 'red'}">{metrics.get('recovery_score', 'N/A')}</span></p>
