...
if __name__ == "__main__":
    ...
    try:
        agent = JarvisAgent(db_path=db_path, dry_run=True, auto_optimise=True)
        ...
        for p in prompts:
            print(f"Prompt : {p!r}")
            result = agent.act(p)
            print(f"Think  : {result.think.decision} (conf={result.think.confidence:.0%})")
            print(f"Outcome: {result.outcome}")
            print(f"Artifact saved: {result.artifact is not None}")
            print()

        print("Reflect:")
        import json as _json
        print(_json.dumps(agent.reflect(), indent=2))

        print("\nStatus:")
        s = agent.status()
        print(f"  tasks={len(s['tasks'])}, artifacts={s['artifacts']}")
