...
class Contact:
    ...
    last_contacted:str = ""
    source:       str = "local"   # "local"|"google"|"prism_memory"
...
class PrismContacts:
    ...
    Sources:
      1. Local SQLite store — always available, PRISM-owned
      2. Google Contacts API — syncs your Google contacts
      3. Auto-extracted from memory — PRISM builds contacts
         from people mentioned in emails, conversations, notes
    ...
    def add(self, contact: Contact) -> str:
        """Store a contact. Returns contact_id."""
    ...
    def search(self, query: str) -> list[Contact]:
        """Find contacts by name, email, organisation, or tag."""
        q = f"%{query.lower()}%"
        with sqlite3.connect(self._db) as c:
            rows = c.execute("""
    ...
    def sync_google(self) -> int:
        """Pull contacts from Google Contacts API. Returns count synced."""
        if not self._google:
            return 0
        url = ("https://people.googleapis.com/v1/people/me/connections"
               "?personFields=names,emailAddresses,phoneNumbers,"
               "organizations&pageSize=200")
        auth = "Bearer " + self._google
    ...
    def extract_from_memory(self, memory, llm_router=None) -> int:
        """
        Scan prism_memory for mentions of people and build contact records.
        Uses LLM to extract structured contact info from natural language.
